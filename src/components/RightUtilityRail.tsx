...
export default function RightUtilityRail({
  ...
  const PLATFORM_TOOLS = [
    { name: "CoT Reasoning", status: cotActive ? "Active" : "Inactive", color: "text-[#6CECC8]" },
  ...
  return (
    <div className="flex flex-col h-full glass-pane rounded-3xl p-3 select-none justify-between overflow-y-auto shadow-sm relative space-y-3 custom-scrollbar">
        ...
        <div className="grid grid-cols-2 gap-1.5 text-[9px] relative">
          ...
          >
            <div className="flex justify-between text-[#9BAAD4]/65 uppercase font-medium">
              <span className="flex items-center gap-0.5">Clarity <HelpCircle className="h-2.5 w-2.5 opacity-40" /></span>
          ...
          >
            <div className="flex justify-between text-[#9BAAD4]/65 uppercase font-medium">
              <span className="flex items-center gap-0.5">Rails <HelpCircle className="h-2.5 w-2.5 opacity-40" /></span>
          ...
          >
            <div className="flex justify-between text-[#9BAAD4]/65 uppercase font-medium">
              <span className="flex items-center gap-0.5 font-sans">Token <HelpCircle className="h-2.5 w-2.5 opacity-40" /></span>
          ...
          >
            <div className="flex justify-between text-[#9BAAD4]/65 uppercase font-medium">
              <span className="flex items-center gap-0.5">Edges <HelpCircle className="h-2.5 w-2.5 opacity-40" /></span>
              <span className="font-mono text-[#EDF2FF]/90 font-bold">{edgeCasesScore}%</span>
            </div>
            <div className="h-0.5 bg-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-amber-400 rounded-full" style={{ width: `${edgeCasesScore}%` }}></div>
            </div>
          </div>
      ...
      <div className="flex-1 space-y-2 min-h-0">
        ...
        <div className="border border-white/5 rounded-xl overflow-hidden bg-[#040910]/20 transition-all">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="text-[7.5px] font-mono bg-emerald-500/10 text-emerald-300 px-1 py-0.5 rounded uppercase font-semibold">
                3 Active
              </span>
              {isSubsystemsOpen ? (
            ...
            <div className="px-2 pb-2.5 pt-0.5 space-y-1 animate-fade-in">
              {PLATFORM_TOOLS.map((t) => (
                <div key={t.name} className="flex justify-between items-center bg-[#040910]/45 px-2 py-1 rounded border border-white/5">
                  ...
                  <span className={`font-mono text-[7.5px] uppercase font-bold tracking-wider ${t.color}`}>
                    {t.status}
                  </span>
                </div>
              ))}
              ...
              {integrations?.github?.connected ? (
                <div className="space-y-1">
                  ...
                  {onOpenSettings && (
                    <button
                      ...
                      className="w-full mt-1 py-1 text-[8px] uppercase tracking-wider bg-white/5 hover:bg-white/10 rounded border border-white/5 text-[#6CECC8] cursor-pointer text-center font-bold"
                    >
                      Repository Rails
                    </button>
                  )}
                ...
                <div className="bg-[#040910]/45 p-2 rounded border border-dashed border-white/10 text-center italic text-white/40 space-y-1.5">
                  ...
                  {onOpenSettings && (
                    <button
                      onClick={onOpenSettings}
                      className="inline-block py-0.5 px-1.5 text-[7px] uppercase font-bold tracking-wider bg-white/5 hover:bg-white/10 text-[#6CECC8] border border-white/5 rounded cursor-pointer transition-all"
                    >
                      Connect in Settings
                    </button>
                  )}
        ...
        {/* F. Agent Monitor / Details Accordion */}
        <div className="border border-white/5 rounded-xl overflow-hidden bg-[#040910]/45 transition-all">
          ...
          >
            <span className="text-[9px] font-mono font-bold text-[#9BAAD4]/75 uppercase tracking-wide flex items-center gap-1.5">
              <Terminal className="h-3.5 w-3.5 text-[#79AEFF]" /> Session Details
            </span>
            <div className="flex items-center gap-2">
          ...
          {isAgentDetailsOpen && (
            <div className="px-2 pb-2.5 pt-0.5 space-y-1 animate-fade-in font-mono text-[8px] leading-tight text-[#9BAAD4]">
              ...
              <div className="flex justify-between items-center bg-[#040910]/45 px-2 py-1 rounded border border-white/5">
                <span className="text-white/30 uppercase text-[7.5px]">Workspace State</span>
              ...
              {activeSession && (
                <div className="space-y-1 mt-1">
                  ...
                  <div className="flex justify-between items-center bg-[#040910]/60 p-1.5 rounded border border-white/5 select-all text-emerald-400 font-bold select-all flex-row">
      ...
      {/* H. Immediate Shortcuts / Quick Actions Grid */}
